					...
					>
						...
						<%
							...
							if (headerNameValue != null) {
								searchAttribute = GetterUtil.getString(request.getParameter(normalizedHeaderName), "");
							}
						%>
						<c:if test='<%=Validator.isNotNull(headerNameValue) && Validator.isNotNull(request.getAttribute("searchTableFilter"))%>'>
							<liferay-portlet:renderURL varImpl="searchURL">
								<portlet:param name="struts_action" value="/search/search" />
							</liferay-portlet:renderURL>
							<div>
                                <form action="<%= searchURL %>" method="get" style="margin: 0">
                                    <input name="<%= namespace %><%= normalizedHeaderName %>"
                                           value="<%= HtmlUtil.escape(searchAttribute) %>"
                                           style="width: 100%"/>
                                </form>
                            </div>
						</c:if>
